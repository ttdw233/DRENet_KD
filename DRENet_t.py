...
class AFM(nn.Module):
    def __init__(self, in_channel):
        ...
        self.conv = convbnrelu(in_channel, in_channel)
        self.conv_1 = convbnrelu(in_channel*3, in_channel)
        self.gcn = GloRe_Unit(in_channel, in_channel)
        self.sigmoid = nn.Sigmoid()
        self.conv3 = DSConv3x3(in_channel, in_channel, dilation=3)
        self.conv5 = DSConv3x3(in_channel, in_channel, dilation=5)
        self.conv7 = DSConv3x3(in_channel, in_channel, dilation=7)

    def forward(self, x):
        x1 = self.conv3(x)
        x2 = self.conv5(x)
        x3 = self.conv7(x)
        x_cat = self.conv_1(torch.cat((x1, x2, x3), dim=1))
        x_conv = self.conv(x)
        x_gcn = self.gcn(x_conv)
        x_out = self.sigmoid(x_gcn) * x_cat + x_conv
        return x_out
